...
* **`Multi Switch (TJ)`.** 그룹 단위 A/B 스위치 노드. 선택 안 된 쪽에 연결된 상위 노드
  체인을 아예 실행 자체에서 빼버린다(단순 Bypass가 아니라, 큐 전송 직전 `app.queuePrompt`를
  가로채 prompt JSON에서 미선택 브랜치를 통째로 가지치기). 모델 로드처럼 무거운 노드가
  A/B 양쪽에 물려 있을 때 선택 안 된 쪽이 VRAM에 올라가는 걸 막는다. 그룹 최대 12개,
  Global/Per-Group 토글 모드 지원.
* **`Batch to MinimaxH3 (TJ)`.** 기존 `Batch to Multi Image Output(TJ)`의 파생 노드 —
  출력 상한을 64→12로 낮추고, 배치 수량을 초과하는 출력은 검정 이미지 대신 `None`을
  리턴하도록 바꿨다. `MiniMax H3 Reference to Video` 같은 Autogrow 계열 노드는 `None`을
  "그 슬롯 미연결"로 알아서 처리하므로, 배치 3장을 넣고 출력 9개를 연결해도 나머지
  6개는 조용히 빈 슬롯 취급된다.
* **PromptDB — 라이브러리 이동/이름변경/백업.** 카드를 다른 라이브러리로 이동(썸네일까지
  같이 이전), 등록된 라이브러리 이름 변경(파일은 그대로), 라이브러리 등록 목록을
  JSON으로 내보내기/가져오기(병합, 이름 충돌 시 자동 접미사) 추가.
* **PromptDB — 썸네일 사이즈 분리.** 갤러리 표시용 썸네일(기본 256px)과 엑셀에 실제로
  임베드되는 이미지(128px 캡, 업스케일 없음)를 독립적으로 조절 가능. 썸네일 교체 시에도
  크기를 직접 지정할 수 있게 필드 추가.
* **PromptDB — 메모를 카드 제목으로.** 메모 필드에 글이 있으면 썸네일 카드 하단에
  프롬프트 대신 메모를 표시(비어 있으면 기존처럼 프롬프트 표시). 검색 대상에도 메모 포함.
* **PromptDB — 정렬 토글.** 갤러리 오름차순/내림차순 정렬 버튼 추가.
* **Ollama LLM Loader (TJ) — 시스템 프롬프트 프리셋 백업.** 사용자 정의 프리셋이
  브라우저 localStorage에만 저장되어 다른 브라우저/PC로 못 옮기던 문제 — Export/Import
  버튼으로 JSON 파일로 내려받고 다른 환경에서 불러올 수 있다.

### [Fixed]

* **`Enhanced KSampler (TJ)` — Krea2 인핸스 기능이 ComfyUI 코어 업데이트 이후 깨지던 문제.**
  코어의 `comfy/ldm/krea2/model.py`가 `forward()`에 `ref_latents` 인자를 추가하면서
  `DIFFUSION_MODEL` wrapper가 7개 positional 인자로 호출되는데, `_krea2_wrapper`는
  6개까지만 받는 고정 시그니처라 `TypeError`가 났다. `*args, **kwargs`로 리팩터링해
  앞으로 코어가 인자를 더 추가해도 깨지지 않게 했다.
* **PromptDB — 상세보기 이미지가 정사각형으로 크롭되던 문제.** 2:3 같은 비율 이미지가
  1:1로 강제 크롭되어 보이던 것을 원본 비율 그대로(`object-fit: contain`) 표시하도록
  변경. 그리드 카드 썸네일 크롭 기준도 센터 → 상단으로 변경(인물 사진에서 얼굴이
  잘리는 문제 완화).
* **LLM 노드 공용 — 일부 GGUF/채팅 핸들러 조합이 `<|channel|>` 같은 채팅 템플릿 제어
  토큰을 소비하지 못하고 그대로 텍스트에 흘려보내던 문제.** `_strip_thinking_tags()`에
  방어적 정규식을 추가해 결과물에서 자동으로 제거되게 했다.
* **플로팅 런처 메뉴 동기화.** `Multi Switch (TJ)`, `Batch to MinimaxH3 (TJ)`가 등록
  누락되어 있던 것을 추가 — 전체 59개 노드가 메뉴와 1:1로 맞음.
...
* **`source_path` → `note`.** 아무도 채우지 않던 "원본 이미지 경로" 필드를, 사용자가
  아무거나 적어 넣는 자유 메모 칸으로 바꿨다. 엑셀 컬럼 라벨도 "원본 경로" → "메모"로
  변경(같은 컬럼, 같은 위치 — 기존 데이터는 그대로, 라벨만 다음 저장 때 자동 갱신).
  Save 입력은 여러 줄 입력으로, Bridge 출력은 와일드카드 타입으로 바뀌어 어디든
  연결할 수 있다.
* 헤더 자동 동기화(`_sync_headers`)를 기록 외에 **편집·삭제·썸네일 교체** 경로에도
  적용 — 갤러리에서만 다루는 워크북도 다음 쓰기 때 라벨이 갱신된다. 조회(`list_rows`)
  경로는 제외(읽기가 파일을 수정하면 안 되므로).
