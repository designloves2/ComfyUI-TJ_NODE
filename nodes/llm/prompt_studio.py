...
from .image_to_prompt import TJ_ImageToPrompt, VISION_TASK_OPTIONS


class TJ_PromptStudio:
    """Prompt Studio (TJ) — 2-in-1 LLM node.
    ...
    def INPUT_TYPES(cls):
        ...
        return {
            "required": {
                ...
                "vision_task": (VISION_TASK_OPTIONS,),
    ...
    def IS_CHANGED(cls, **kwargs):
        if kwargs.get("lock_in"):
            excluded = {"seed", "clip", "image", "get_name", "get_name_prompt", "get_name_image"}
            return "LOCKED|" + "|".join(str(kwargs.get(k, "")) for k in sorted(kwargs) if k not in excluded)
        return float("nan")
    ...
    def run(self, get_name_prompt="(none)", get_name_image="(none)", get_name="(none)",
            set_name="Prompt_Studio", mode="Auto", raw_prompt="",
            ...
            purpose="Image", vision_task="Caption (plain description)",
            model_format="Universal Natural Language", aesthetic="None (no aesthetic injection)",
            extra_instructions="", system_prompt_override="", custom_instruction="",
            append_no_think=True, n_gpu_layers=-1, n_ctx=4096, max_tokens=1000,
            temperature=0.7, top_p=0.9, repeat_penalty=1.15, seed=0, lock_in=False,
            raw_prompt_input=None, image=None, clip=None):
        ...
        if effective_mode == "Image to Prompt":
            if image is None:
                raise ValueError("Prompt Studio (TJ): Image input is required for Image to Prompt mode.")
            return TJ_ImageToPrompt().describe(
                get_name=get_name_image if get_name_image != "(none)" else get_name,
                set_name=set_name, image=image, model_backend=model_backend,
                gguf_model=gguf_model, mmproj_file=mmproj_file, chat_handler=chat_handler,
                text_encoder_name=text_encoder_name, clip_loader_type=clip_loader_type,
                vision_task=vision_task, model_format=model_format, aesthetic=aesthetic,
                custom_instruction=custom_instruction, n_gpu_layers=n_gpu_layers,
                n_ctx=n_ctx, max_tokens=max_tokens, temperature=temperature,
                seed=seed, lock_in=lock_in, clip=clip,
            )

        return TJ_PromptEnhancer().enhance(
            get_name=get_name_prompt if get_name_prompt != "(none)" else get_name,
            set_name=set_name, raw_prompt=raw_prompt, model_backend=model_backend,
            gguf_model=gguf_model, mmproj_file=mmproj_file, text_encoder_name=text_encoder_name,
            clip_loader_type=clip_loader_type, purpose=purpose, model_format=model_format,
            aesthetic=aesthetic, extra_instructions=extra_instructions,
            system_prompt_override=system_prompt_override, append_no_think=append_no_think,
            n_gpu_layers=n_gpu_layers, n_ctx=n_ctx, max_tokens=max_tokens,
            temperature=temperature, top_p=top_p, repeat_penalty=repeat_penalty,
            seed=seed, lock_in=lock_in, raw_prompt_input=raw_prompt_input, clip=clip,
        )
