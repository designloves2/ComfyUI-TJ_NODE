...
class TJ_MultiModelSelecter:
    ...
    def select(self, auto_set=False, output_mode="Model Direct out", select_type="Model", slots_json="[]", clip_types_json="[]", unique_id=None, prompt=None):
        ...
        # If ComfyUI does not provide PROMPT for any reason, keep the old safe fallback:
        # load selected visible slots. When PROMPT is available, Direct out loads only
        # outputs that are actually connected/requested.
...
NODE_CLASS_MAPPINGS = {"TJ_MultiModelSelecter": TJ_MultiModelSelecter}
NODE_DISPLAY_NAME_MAPPINGS = {"TJ_MultiModelSelecter": "Multi Model Selecter (TJ)"}
