// web/ltx25_clip_gguf_tj.js
// LTX25 CLIP GGUF LOADER (TJ)
// 단일 CLIP 출력 로더. TJ 무선 Set(setnode_name 동기화 + Auto Set)만 붙인다 —
// 입력 소켓이 없으므로 embedded Get(get_name)은 없다.
// Auto Set 규약은 web/tj_resolution.js 와 동일: properties.auto_sets[slot] = 이름,
// 출력 라벨 = 이름 + " ▶", 등록 후 Get/MultiGet 에 갱신 신호.
// (노드 타입은 web/set_getnode_tj.js 의 AUTO_SET_PROVIDER_TYPES 에도 등록됨)

import { app } from "../../scripts/app.js";

const NODE_CLASS = "TJ_LTX25ClipLoaderGGUF";
const OUTPUT_NAMES = ["clip"];
...
function updateAutoSets(node) {
    ...
    const base = String(setW?.value || node.title || "LTX25_CLIP").trim();
    ...
    (node.outputs || []).forEach((out, i) => {
        ...
        // 단일 출력이라 setnode_name 을 그대로 쓰되, 중복이면 _1, _2 …
...
function installAutoSet(node) {
    ...
    if (autoW && !autoW._tj_ltx25_attached) {
        autoW._tj_ltx25_attached = true;
    ...
    if (setW && !setW._tj_ltx25_name_attached) {
        setW._tj_ltx25_name_attached = true;
